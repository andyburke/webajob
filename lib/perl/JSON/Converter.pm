...
sub toJson
{
  ...
  if ( my $package = blessed($obj) )
  {
    warn("trying to convert a blessed object ($package); assuming it's a hash and trying as hard as I possibly can");
  }
...
sub hashToJson
{
  ...
  if ( grep { refaddr($_) == refaddr($obj) } @{ $self->{_stack_myself} } )
  {
    die "circle ref!";
  }
  ...
  {
    my $del = $self->{_delstr};
    return "{$pre" . join( ",$pre", map { _stringfy($_) . $del . $res{$_} } keys %res ) . "$post}";
  }
  else
  {
...
sub _initConvert
{
  ...
  $self->{_delstr} = $self->{delimiter} ? ( $self->{delimiter} == 1 ? ': ' : ' : ' ) : ':';
